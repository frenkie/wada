    ...
    namespace Scripting
    {
        namespace Effects
        {
            ...
            public abstract class ParticleAffector : MonoBehaviour
            {
                ...
                [Header( "Common Controls" )] public float radius = Mathf.Infinity;
                public float force = 5.0f;

                public Vector3 offset = Vector3.zero;

                public float scaledRadius => radius * transform.lossyScale.x;
                ...
                public AnimationCurve scaleForceByDistance = new(
                    ...
                    for ( int i = 0; i < particleSystemsCount; i++ )
                    {
                        ...
                        if ( simulationSpace == ParticleSystemSimulationSpace.World )
                        {
                            for ( int j = 0; j < particleCount; j++ )
                            {
                                ...
                                parameters.scaledDirectionToAffectorCenter.x =
                                    transformPosition.x - parameters.particlePosition.x;
                                parameters.scaledDirectionToAffectorCenter.y =
                                    transformPosition.y - parameters.particlePosition.y;
                                parameters.scaledDirectionToAffectorCenter.z =
                                    transformPosition.z - parameters.particlePosition.z;

                                parameters.distanceToAffectorCenterSqr =
                                    parameters.scaledDirectionToAffectorCenter.sqrMagnitude;

                                if ( parameters.distanceToAffectorCenterSqr < radiusSqr )
                                {
                                    float distanceToCenterNormalized =
                                        parameters.distanceToAffectorCenterSqr / radiusSqr;
                                    float distanceScale = scaleForceByDistance.Evaluate( distanceToCenterNormalized );

                                    Vector3 force = GetForce();
                                    float forceScale = forceDeltaTime * distanceScale *
                                                       particleSystemExternalForcesMultipliers[i];
                                        ...
                                        switch ( scalingMode )
                                        {
                                            case ParticleSystemScalingMode.Hierarchy:
                                            {
                                                parameters.particlePosition =
                                                    simulationSpaceTransform.TransformPoint(
                                                        particleSystemParticles[i][j].position );
                                            ...
                                            case ParticleSystemScalingMode.Local:
                                            {
                                                ...
                                                parameters.particlePosition =
                                                    Vector3.Scale( parameters.particlePosition,
                                                        particleSystemLocalScale );
                                                parameters.particlePosition =
                                                    particleSystemRotation * parameters.particlePosition;
                                                parameters.particlePosition =
                                                    parameters.particlePosition + particleSystemPosition;
                                    ...
                                    switch ( simulationSpace )
                                    {
                                        ...
                                        case ParticleSystemSimulationSpace.Custom:
                                        {
                                            switch ( scalingMode )
                                            {
                                                ...
                                                {
                                                    throw new System.NotSupportedException(
                                                        string.Format( "Unsupported scaling mode '{0}'.",
                                                            scalingMode ) );
                                                }
                                            }
