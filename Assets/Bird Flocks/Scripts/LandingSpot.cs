...
public class LandingSpot : MonoBehaviour
{
    ...
    public void OnDrawGizmos()
    {
        ...
        Gizmos.DrawCube( new Vector3( _thisT.position.x, _thisT.position.y, _thisT.position.z ),
            Vector3.one * _controller._gizmoSize );
        Gizmos.DrawCube( _thisT.position + _thisT.forward * _controller._gizmoSize,
            Vector3.one * _controller._gizmoSize * .5f );
    ...
    public void LateUpdate()
    {
        ...
        else if ( landingChild.Paused )
        {
            return; //?? Does this help with letting it be held?
        }
        ...
        if ( _controller._flock.gameObject.activeInHierarchy && landing && landingChild != null )
        {
            ...
            float distance = Vector3.Distance( landingChild._thisT.position,
                _thisT.position + landingChild._landingPosOffset );
            //Start landing if distance is close enough
            if ( distance < 5 && distance > .5f )
            {
                if ( _controller._soarLand )
                {
                    landingChild.SoarAnimation();
                    if ( distance < 2 )
                    {
                        landingChild.FlapAnimation();
                    }
                }
            ...
            else if ( distance <= .5f )
            {
                ...
                if ( distance > _controller._snapLandDistance )
                {
                    ...
                    landingChild._thisT.position +=
                        (_thisT.position + landingChild._landingPosOffset - landingChild._thisT.position) *
                        Time.deltaTime * landingChild._speed * _controller._landingSpeedModifier * 2;
                }
                else
                {
                    _gotcha = true;
    ...
    {
        if ( landingChild._thisT.eulerAngles.x == 0 )
        {
            return;
        }
    ...
    public void RotateBird()
    {
        ...
        // TODO: Frank : I disabled the Y rotation, but added the X. I'm more interested in the straightness of the bird
        rotE.x = Mathf.LerpAngle( landingChild._thisT.rotation.eulerAngles.x, _thisT.rotation.eulerAngles.x,
            lerpCounter * Time.deltaTime * _controller._landedRotateSpeed );
        //rotE.y = Mathf.LerpAngle( landingChild._thisT.rotation.eulerAngles.y, _thisT.rotation.eulerAngles.y,
        //    lerpCounter * Time.deltaTime * _controller._landedRotateSpeed );
    ...
    public IEnumerator GetFlockChild(float minDelay, float maxDelay)
    {
        ...
        if ( _controller._flock.gameObject.activeInHierarchy && landingChild == null )
        {
            ...
            for ( int i = 0; i < _controller._flock._roamers.Count; i++ )
            {
                ...
                if ( !child._landing && !child._dived && child.CanLand )
                {
                    if ( !_controller._onlyBirdsAbove )
                    {
                        if ( fChild == null &&
                             _controller._maxBirdDistance >
                             Vector3.Distance( child._thisT.position, _thisT.position ) &&
                             _controller._minBirdDistance < Vector3.Distance( child._thisT.position, _thisT.position ) )
                        {
                            fChild = child;
                        ...
                        else if ( fChild != null && Vector3.Distance( fChild._thisT.position, _thisT.position ) >
                                 Vector3.Distance( child._thisT.position, _thisT.position ) )
                        {
                            fChild = child;
                        }
                    ...
                    {
                        if ( fChild == null && child._thisT.position.y > _thisT.position.y &&
                             _controller._maxBirdDistance >
                             Vector3.Distance( child._thisT.position, _thisT.position ) &&
                             _controller._minBirdDistance < Vector3.Distance( child._thisT.position, _thisT.position ) )
                        {
                        ...
                        else if ( fChild != null && child._thisT.position.y > _thisT.position.y &&
                                  Vector3.Distance( fChild._thisT.position, _thisT.position ) >
                                  Vector3.Distance( child._thisT.position, _thisT.position ) )
                        {
                            fChild = child;
                        }
            ...
            if ( fChild != null )
            {
                ...
                landingChild._lander = this;
                if ( _controller._autoDismountDelay.x > 0 )
                {
                    Invoke( "ReleaseFlockChild",
                        Random.Range( _controller._autoDismountDelay.x, _controller._autoDismountDelay.y ) );
                }
    ...
    public void ReleaseFlockChild()
    {
        ...
        if ( _controller._flock.gameObject.activeInHierarchy && landingChild != null && !landingChild.Paused )
        {
            _gotcha = false;
            lerpCounter = 0;
            ...
            landingChild._lander = null;
            landingChild.Flap();
            landingChild._wayPoint =
                new Vector3( landingChild._wayPoint.x, _thisT.position.y + 10, landingChild._wayPoint.z );

            if ( _controller._autoCatchDelay.x > 0 )
            {
                StartCoroutine( GetFlockChild( _controller._autoCatchDelay.x + 0.1f,
                    _controller._autoCatchDelay.y + 0.1f ) );
            }
