...
{
    [HideInInspector] public LandingSpot _lander; //Reference to the flock controller that spawned this bird
    ...
    [HideInInspector] public bool _dived = true; //Indicates if this bird has recently performed a dive movement
    ...
    public bool AutomaticWaypoints = true;
    ...
    public Transform
        _modelT; // Reference to bird model transform (caching tranform to avoid any extra getComponent calls)
    ...
    [HideInInspector] public float _avoidDistance; //How far from an obstacle this can be before starting to avoid it
    float _soarTimer;
    bool _instantiated;
    ...
    public bool AlignWithWaypointFromStart = true;
    public bool SetRandomScaleAtStart = true;
    public float WaypointDistanceOverride = -1;
    ...
    {
        //Skip frames
        if ( !Paused && _spawner != null && (_spawner._updateDivisor <= 1 || _spawner._updateCounter == _updateSeed) )
        {
            SoarTimeLimit();
    ...
    public void OnEnable()
    {
        if ( _instantiated )
        {
            ...
            if ( _landing )
            {
                _animator.SetTrigger( _spawner._idleAnimation );
            }
            else
            {
                _animator.SetTrigger( _spawner._flapAnimation );
            }
        }
    }
    ...
    {
        if ( _soar && _spawner._soarMaxTime > 0 )
        {
    ...
    {
        float closeToWaypointMagnitude = (_thisT.position - _wayPoint).magnitude;
        float waypointDistanceThreshold = GetWaypointDistanceThreshold();

        if ( !AutomaticWaypoints && closeToWaypointMagnitude < waypointDistanceThreshold )
        {
            Paused = true;
        }
        else if ( !_landing && closeToWaypointMagnitude < waypointDistanceThreshold + _stuckCounter )
        {
            if ( AutomaticWaypoints )
            {
                Wander( 0.0f );
    ...
    float GetWaypointDistanceThreshold()
    {
        return WaypointDistanceOverride > -1 ? WaypointDistanceOverride : _spawner._waypointDistance;
    }
    ...
    public bool Avoidance()
    {
        ...
        rotE.x = 0;
        if ( Physics.Raycast( _thisT.position, fwd + _modelT.right * _avoidValue, out hit, _avoidDistance,
                _spawner._avoidanceMask ) )
        {
        ...
        else if ( Physics.Raycast( _thisT.position, fwd + _modelT.right * -_avoidValue, out hit, _avoidDistance,
                     _spawner._avoidanceMask ) )
        {
            rotE.y += _spawner._birdAvoidHorizontalForce * _spawner._newDelta * _damping;
            rot.eulerAngles = rotE;
        ...
        if ( _spawner._birdAvoidDown && !_landing && Physics.Raycast( _thisT.position, -Vector3.up, out hit,
                _avoidDistance, _spawner._avoidanceMask ) )
        {
        ...
        else if ( _spawner._birdAvoidUp && !_landing && Physics.Raycast( _thisT.position, Vector3.up, out hit,
                     _avoidDistance, _spawner._avoidanceMask ) )
        {
            //rotE.x += _spawner._birdAvoidVerticalForce * _spawner._newDelta * _damping;
            rot.eulerAngles = rotE;
    ...
    public void LimitRotationOfModel()
    {
        ...
        if ( (((_soar && _spawner._flatSoar) || (_spawner._flatFly && !_soar)) && _wayPoint.y > _thisT.position.y) ||
             _landing )
        {
            rotE.x = Mathf.LerpAngle( _modelT.localEulerAngles.x, -_thisT.localEulerAngles.x,
                _spawner._newDelta * 1.75f );
            rot.eulerAngles = rotE;
    ...
    {
        _animator.ResetTrigger( _spawner._flapAnimation );
        _animator.SetTrigger( _spawner._idleAnimation );
    }
    ...
    public void Soar()
    {
        if ( _move )
        {
            //if(this._model != null) _model.GetComponent<Animation>().CrossFade(_spawner._soarAnimation, 1.5f);
            _animator.SetTrigger( _spawner._soarAnimation );
            if ( AutomaticWaypoints )
            {
                _wayPoint = findWaypoint();
            }
    ...
    public void Dive()
    {
        ...
        else
        {
            if ( _model != null )
            {
                // TODO you need to set a parameter that is used in the Animator Controller for the speed

                // foreach(AnimatorStateInfo state in _model.GetComponent<Animation>()) {
