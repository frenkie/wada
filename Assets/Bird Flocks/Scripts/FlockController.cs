...
public class FlockController : MonoBehaviour
{
    ...
    public float
        _minDamping =
            1.0f; // Rotation tween damping, lower number = smooth/slow rotation (if this get stuck in a loop, increase this value)

    public float _maxDamping = 2.0f;

    public float
        _waypointDistance =
            1.0f; // How close this can get to waypoint before creating a new waypoint (also fixes stuck in a loop)
    ...
    public float
        _positionSphere =
            25.0f; // If _randomPositionTimer is bigger than zero the controller will be moved to a random position within this sphere
    ...
    public int
        _updateDivisor =
            1; //Skip update every N frames (Higher numbers might give choppy results, 3 - 4 on 60fps , 2 - 3 on 30 fps)
    ...
    public bool KeepChildAmountUpdated = true;
    ...
    public void Start()
    {
        ...
        {
            InvokeRepeating( "SetFlockRandomPosition", _randomPositionTimer, _randomPositionTimer ); // > C
        }
    }
    ...
    public void AddChild(FlockChild child)
    {
        if ( _groupChildToNewTransform )
        {
            InstantiateGroup();
        ...
        child._spawner = this;
        _roamers.Add( child );
        AddChildToParent( child.transform );
    }
    ...
    public void InstantiateGroup()
    {
        ...
        g.name = _thisT.name + " Flock Container";
    }
    ...
    public void OnDrawGizmos()
    {
        ...
        Gizmos.DrawWireCube( _posBuffer,
            new Vector3( _spawnSphere * 2, _spawnSphereHeight * 2, _spawnSphereDepth * 2 ) );
        Gizmos.color = Color.cyan;
        Gizmos.DrawWireCube( _thisT.position,
            new Vector3( _positionSphere * 2 + _spawnSphere * 2, _positionSphereHeight * 2 + _spawnSphereHeight * 2,
                _positionSphereDepth * 2 + _spawnSphereDepth * 2 ) );
    }
    ...
    public void SetFlockRandomPosition()
    {
        ...
        if ( _forceChildWaypoints )
        {
            ...
            {
                if ( _roamers[i].AutomaticWaypoints )
                {
