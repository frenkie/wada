namespace NatureManufacture.RAM.Editor
{
    ...
    public sealed class RamSplineEditor : Editor
    {
        ...
        private void ParentingSplineUI()
        {
            GUILayout.Label("Rivers connections", EditorStyles.boldLabel);


            _ramSpline.beginningSpline =
                (RamSpline)EditorGUILayout.ObjectField("Beginning spline", _ramSpline.beginningSpline, typeof(RamSpline),
                    true);

            if (_ramSpline.beginningSpline == _ramSpline)
                _ramSpline.beginningSpline = null;

            _ramSpline.endingSpline =
                (RamSpline)EditorGUILayout.ObjectField("Ending spline", _ramSpline.endingSpline, typeof(RamSpline), true);
            if (_ramSpline.endingSpline == _ramSpline)
                _ramSpline.endingSpline = null;


            if (_ramSpline.beginningSpline != null)
            {
                if (_ramSpline.NmSpline.MainControlPoints.Count > 0 && _ramSpline.beginningSpline.NmSpline.Points.Count > 0)
                {
                    _ramSpline.beginningMinWidth = _ramSpline.beginningMinWidth * (_ramSpline.beginningSpline.BaseProfile.vertsInShape - 1);
                    _ramSpline.beginningMaxWidth = _ramSpline.beginningMaxWidth * (_ramSpline.beginningSpline.BaseProfile.vertsInShape - 1);
                    EditorGUILayout.MinMaxSlider("Part parent", ref _ramSpline.beginningMinWidth, ref _ramSpline.beginningMaxWidth,
                        0, _ramSpline.beginningSpline.BaseProfile.vertsInShape - 1);
                    _ramSpline.beginningMinWidth = (int)_ramSpline.beginningMinWidth;
                    _ramSpline.beginningMaxWidth = (int)_ramSpline.beginningMaxWidth;
                    _ramSpline.beginningMinWidth =
                        Mathf.Clamp(_ramSpline.beginningMinWidth, 0, _ramSpline.beginningSpline.BaseProfile.vertsInShape - 1);
                    _ramSpline.beginningMaxWidth =
                        Mathf.Clamp(_ramSpline.beginningMaxWidth, 0, _ramSpline.beginningSpline.BaseProfile.vertsInShape - 1);
                    if (_ramSpline.beginningMinWidth == _ramSpline.beginningMaxWidth)
                    {
                        if (_ramSpline.beginningMinWidth > 0)
                            _ramSpline.beginningMinWidth--;
                        else
                            _ramSpline.beginningMaxWidth++;
                    }

                    _ramSpline.BaseProfile.vertsInShape = (int)(_ramSpline.beginningMaxWidth - _ramSpline.beginningMinWidth) + 1;
                    _ramSpline.beginningMinWidth = _ramSpline.beginningMinWidth / (_ramSpline.beginningSpline.BaseProfile.vertsInShape - 1);
                    _ramSpline.beginningMaxWidth = _ramSpline.beginningMaxWidth / (_ramSpline.beginningSpline.BaseProfile.vertsInShape - 1);

                    _ramSpline.GenerateBeginningPointsFromParent();
                }
            }
            else
            {
                _ramSpline.beginningMaxWidth = 1;
                _ramSpline.beginningMinWidth = 0;
            }


            if (_ramSpline.endingSpline != null)
            {
                if (_ramSpline.NmSpline.MainControlPoints.Count > 1 && _ramSpline.endingSpline.NmSpline.Points.Count > 0)
                {
                    _ramSpline.endingMinWidth = _ramSpline.endingMinWidth * (_ramSpline.endingSpline.BaseProfile.vertsInShape - 1);
                    _ramSpline.endingMaxWidth = _ramSpline.endingMaxWidth * (_ramSpline.endingSpline.BaseProfile.vertsInShape - 1);

                    EditorGUILayout.MinMaxSlider("Part parent", ref _ramSpline.endingMinWidth, ref _ramSpline.endingMaxWidth, 0,
                        _ramSpline.endingSpline.BaseProfile.vertsInShape - 1);

                    _ramSpline.endingMinWidth = (int)_ramSpline.endingMinWidth;
                    _ramSpline.endingMaxWidth = (int)_ramSpline.endingMaxWidth;
                    _ramSpline.endingMinWidth = Mathf.Clamp(_ramSpline.endingMinWidth, 0, _ramSpline.endingSpline.BaseProfile.vertsInShape - 1);
                    _ramSpline.endingMaxWidth = Mathf.Clamp(_ramSpline.endingMaxWidth, 0, _ramSpline.endingSpline.BaseProfile.vertsInShape - 1);
                    if (_ramSpline.endingMinWidth == _ramSpline.endingMaxWidth)
                    {
                        if (_ramSpline.endingMinWidth > 0)
                            _ramSpline.endingMinWidth--;
                        else
                            _ramSpline.endingMaxWidth++;
                    }

                    _ramSpline.BaseProfile.vertsInShape = (int)(_ramSpline.endingMaxWidth - _ramSpline.endingMinWidth) + 1;
                    _ramSpline.endingMinWidth = _ramSpline.endingMinWidth / (_ramSpline.endingSpline.BaseProfile.vertsInShape - 1);
                    _ramSpline.endingMaxWidth = _ramSpline.endingMaxWidth / (_ramSpline.endingSpline.BaseProfile.vertsInShape - 1);

                    _ramSpline.GenerateEndingPointsFromParent();
                }
            }
            else
            {
                _ramSpline.endingMaxWidth = 1;
                _ramSpline.endingMinWidth = 0;
            }
        }
