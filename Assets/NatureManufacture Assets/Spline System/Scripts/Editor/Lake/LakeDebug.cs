        ...
        public void ShowDebugHandles()
        {
            ...
            if (mesh)
            {
                ...
                for (int i = 0; i < _vertices.Count; i++)
                {
                    ...
                    if (debugUV0)
                    {
                        Handles.ArrowHandleCap(0, _vertices[i]+(debugFlowmap? new Vector3(0,0.1f,0):Vector3.zero), Quaternion.LookRotation(new Vector3(_uv0[i].z, 0, _uv0[i].w).normalized), 0.2f, EventType.Repaint);
